import type { createPublishRoutes } from "../server/routes.js";
...
type ApiOutputs = { publisher: inferRouterOutputs<PublisherRouter> };
...
export type PublisherTrpcClient = {
  publisher: Pick<TRPCClient<PublisherRouter>, keyof inferRouterOutputs<PublisherRouter>>;
};

export type ServerBackedPublisherGatewayOptions = {
  fetch?: typeof globalThis.fetch;
  captureOperation?: () => { fetch: (input: RequestInfo | URL, init?: RequestInit, implementation?: typeof globalThis.fetch) => Promise<Response> };
...
export function createServerBackedPublisherGateway(
  ...
  const captureOperation = options.captureOperation ?? (() => ({ fetch: (input: RequestInfo | URL, init?: RequestInit) => rawFetch(input, init) }));
  const fetchImpl: typeof globalThis.fetch = (input, init) =>
    captureOperation().fetch(input, init, rawFetch);
  ...
  return {
    ...
    preflightDraft(draftId, signal) {
      ...
      return translateGatewayErrors(async () => {
        ...
        const [preflight, billing] = await Promise.all([
          ...
          client.publisher.dashboard.query(undefined, { signal }).then(dashboard => dashboard.wallet),
        ]);
    ...
    batchCsvUrl(batchId) {
      return (options.resourceUrl ?? ((path: string) => path))(
