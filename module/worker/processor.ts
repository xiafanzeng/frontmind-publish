...
import type { PrivateObjectReader, PrivateObjectStore } from "./ports.js";
import {
  DocxSafetyError,
...
export class PublisherWorkerProcessor {
  ...
  constructor(
    ...
    const mediaFetcher = dependencies.mediaFetcher ?? (async () => {
      throw new Error("Publisher media fetcher is not configured");
    });
    this.logoArchiveCache = new PublisherLogoArchiveCache(dependencies.objectStore, mediaFetcher, () => this.now().valueOf());
  }
  ...
  private async submitItem(
    ...
    try {
      ...
      try {
        if (
          isKolSubmissionUnknownError(error) ||
...
export function publisherMediaLogoErrorCode(error: unknown): string {
  if (errorName(error) === "RemoteMediaRejectedError") return "remote_rejected";
  if (errorName(error) === "RemoteMediaFetchError") return "remote_fetch_failed";
  if (error instanceof PublisherImageError)
    return error.code === "logo_too_large"
      ? "normalized_logo_too_large"
      : "image_normalization_failed";
  if (errorName(error) === "ObjectStoreError") return "object_store_failed";
...
function isKolProviderError(error: unknown): error is { code: string; message: string; submissionMayHaveSucceeded?: boolean; details?: { operation?: string; httpStatus?: number } } {
  return Boolean(error && typeof error === "object" && typeof (error as { code?: unknown }).code === "string");
}
function isKolSubmissionUnknownError(error: unknown): boolean {
  return errorName(error) === "KolSubmissionUnknownError";
}
