...
import { publisherCanonicalImageReferences } from "./domain/canonical-html.js";
import { publisherImageCapabilityEvidenceBlocker } from "./publisher-capability.js";
import { publisherMoneyFromApiString, publisherMoneyToApiString } from "./money-format.js";
import { RepositoryError } from "@frontmind/module-contracts/errors";
import type { PublishApiCore, PublishDatabase as Database, PublishTransaction as Transaction } from "./persistence-core.js";
...
export function createPublishingPersistence(core: PublishApiCore) {
const { monitoringProjectOwnerPredicate, monitoringEnterpriseProjectIdForOwner, assertMonitoringEnterpriseProjectActive, settlePublisherItemMoney, reactivatePublisherItemReservation, derivePublisherBatchFundsStatus } = core;
const { mediaPublishingItemPriceSnapshots, publisherArticleAssets, publisherArticleVersionAssets, publisherArticleVersions, publisherArticles, publisherBatches, publisherDocxImports, publisherDraftItems, publisherDrafts, publisherItems, publisherJobs, publisherLiveWhitelist, publisherMediaCapabilities, publisherMediaLogoAssets, publisherMediaLogoResolutions, publisherMediaResources, publisherObjectLeases, publisherPreflights, publisherMediaSyncRuns, publisherReconciliationCandidates, publisherRuntimeState, publisherSubmissionAttempts } = core.tables;
class PublishingRepository {

  constructor(public readonly db: Database) {}
  mediaFacetsCache = new Map<
    string,
    {
      expiresAt: number;
  ...
  async createPublisherObjectLease(
    ...
    await core.assertOwner(this.db, ownerId);
  ...
  async createDocxImport(
    ...
    return this.db.transaction(async (tx) => {
      ...
      await core.assertOwner(tx, ownerId);
      const id = randomUUID();
      const now = new Date();
  ...
  async loadPublisherMediaFacets(
  ...
  async getPublisherDashboard(ownerId: string) {
    const wallet = await core.billingSummary(this.db, ownerId);
  ...
  async listPublisherUnknownItems(limit = 100) {
    const rows = await this.db
      ...
      .limit(Math.min(Math.max(limit, 1), 100));
    return Promise.all(rows.map(async row => ({ ...row, username: await core.ownerDisplayName(this.db, row.item.ownerId) })));
  }
  ...
  async savePublisherDraft(
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (!input.draftId && input.idempotencyKey) {
        ...
        await core.assertOwner(tx, ownerId, true);
        const receipt = await core.readAudit(tx, id, ownerId);
        if (receipt) {
          if (
  ...
  async preflightPublisherDraft(
    ...
    await core.ensureWallet(this.db, ownerId);
    return this.db.transaction(async (tx) => {
      await assertMonitoringEnterpriseProjectActive(tx, monitoringEnterpriseProjectIdForOwner(ownerId), ownerId);
  ...
  async submitPublisherDraft(
    ...
    return this.db.transaction(async (tx) => {
      ...
      await core.lockWallet(tx, ownerId);
      const [replay] = await tx
        .select()
        .from(publisherBatches)
        .where(
      ...
      const nextWallet = await core.quoteReservation(tx, ownerId, total);
      const [draft] = await tx
        .select()
        .from(publisherDrafts)
        .where(
      ...
      await core.createReservation(tx, {
        id: reservationId,
        ownerId,
        batchId,
      ...
      for (const selected of selectedItems) {
        ...
        await core.createItemSettlement(tx, {
          itemId,
          ownerId,
          reservationId,
          amountTenThousandths: price,
      ...
      await core.commitReservation(tx, { ownerId, total, reservationId, batchId, nextWallet });
      await tx
        .update(publisherDrafts)
...
async function computePreflight(
  ...
  const available = await core.availableFunds(tx, ownerId);
...
function publisherLiveImageCanaryBlocker(input: {
...
function publisherFrozenImageEvidenceBlocker(input: {
  containsImages: boolean;
...
function publisherPreflightTokenBlocker(input: {
  preflight:
    | {
        ownerId: string;
        draftId: string;
...
function publisherCatalogKindIsIncomplete(
  mode: PublicationMode,
...
function publisherCatalogResourceRevisionIsPublishable(input: {
...
function publisherRealLogoCoverage(input: {
  total: number;
  logoProviderArchived: number;
  logoIconArchived: number;
  logoWebSearchVerifiedArchived: number;
...
function publisherMediaEditorialMetadata(
...
return { PublishingRepository, publisherLiveImageCanaryBlocker, publisherFrozenImageEvidenceBlocker, publisherPreflightTokenBlocker, publisherCatalogKindIsIncomplete, publisherCatalogResourceRevisionIsPublishable, publisherRealLogoCoverage, publisherMediaEditorialMetadata };
}
