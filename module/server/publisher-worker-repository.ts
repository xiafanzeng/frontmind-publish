...
import { publisherPriceToTenThousandths } from "./money-format.js";
import { RepositoryError } from "@frontmind/module-contracts/errors";
import type { PublishPersistenceCore, PublishDatabase as Database, PublishTransaction as Transaction } from "./persistence-core.js";
...
export function createPublishWorkerPersistence(core: PublishPersistenceCore) {
const { mediaPublishingItemPriceSnapshots, publisherArticleAssets, publisherArticleVersionAssets, publisherArticleVersions, publisherArticles, publisherBatches, publisherDocxImports, publisherItems, publisherJobs, publisherLiveWhitelist, publisherMediaCapabilities, publisherMediaLogoAssets, publisherMediaLogoResolutions, publisherMediaResources, publisherMediaSyncRuns, publisherMediaSyncStaging, publisherObjectLeases, publisherReconciliationCandidates, publisherRuntimeState, publisherSubmissionAttempts, publisherSubmissionGate } = core.tables;
const { settlePublisherItemMoney, derivePublisherBatchFundsStatus } = core;
const CATALOG_SYNC_GATE_ID = "kol-catalog-sync";

class PublisherWorkerRepository {
  ...
  async catalogRevisionForStaging(runId: string) {
  ...
  async finishKnownSubmission(input: {
    itemId: string;
    attemptId: string;
    reason: string;
    providerRaw?: unknown;
...
function publisherRealRuntimeGateBlocker(input: {
...
function publisherLiveCanaryBlocker(input: {
...
function publisherLiveImageEvidenceBlocker(input: {
  containsImages: boolean;
...
function publisherReportedPriceMismatch(
...
function publisherStatusForProcessingObservation(
 ...
 */
function publisherMediaLogoCandidatePlan(input: {
  name: string;
...
function publisherCatalogSyncSlot(at: Date) {
...
function publisherDocxImageLeaseOperationId(importId: string) {
  return `publisher:docx-images:${importId}`;
}
return { PublisherWorkerRepository, publisherRealRuntimeGateBlocker, publisherLiveCanaryBlocker, publisherLiveImageEvidenceBlocker, publisherReportedPriceMismatch, publisherStatusForProcessingObservation, normalizeCatalogResource, publisherMediaLogoCandidatePlan, publisherCatalogSyncSlot, publisherDocxImageLeaseOperationId };
}
