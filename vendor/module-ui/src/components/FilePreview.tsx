...
import { deliveryProjectHeaders, sanitizeBrandText, usesModuleFileTransport } from "./file-preview-runtime";
...
async function fetchFileAsBlob(
  ...
  if (usesModuleFileTransport()) {
    try { moduleUrl = moduleFileContentUrl(source); }
    catch (error) { throw new FileContentRequestError(error instanceof Error ? error.message : "文件来源不可用", {code: "SOURCE_FORBIDDEN"}); }
  }
  const url = moduleUrl ?? (source.kind === "owned_file"
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      if (previewSource?.kind === "owned_file") {
        if (usesModuleFileTransport() || managedLocalAssetContentUrl(previewSource.fileId)) {
          const blobUrl = await fetchFileAsBlobWithRetry(
            previewSource,
            downloadName,
          );
