...
import { useModulePresentation, type WorkbenchAction, type WorkbenchModulePresentation } from "../dashboard/module-presentation";
import { WORKBENCH_AUX_WIDTHS } from "../dashboard/workbench-presentation";
import "./AgentWorkbenchShell.css";
import "./resource-pane-scroll.css";
export type { WorkbenchAction } from "../dashboard/module-presentation";
export type AgentWorkbenchShellProps = {
  workspaceModule?: WorkbenchModulePresentation | null;
  moduleColor?: string;
  ...
  taskKey?: string;
  scrollMain?: boolean;
  /** Static readers retain their scroll position without a chat jump control. */
  showLatestControl?: boolean;
  auxiliaryScroll?: boolean;
  /** Resource readers opt in to independent desktop panes without altering chat layouts. */
  independentResourceScroll?: boolean;
...
export function AgentWorkbenchShell({
  workspaceModule,
  moduleColor: suppliedModuleColor,
  requestNavigation,
  projectId,
  moduleId,
  title,
  ...
  taskKey,
  scrollMain,
  showLatestControl = true,
  auxiliaryScroll = true,
  independentResourceScroll = false,
  conversation,
  children,
  result,
  resultTitle = "任务信息",
  ...
  const module = workspaceModule ?? presentation?.module;
  const moduleColor = suppliedModuleColor ?? module?.color;
  const requestWorkspaceNavigation = requestNavigation ?? presentation?.requestNavigation ?? ((action: () => void) => action());
  ...
  const [resourceContentWidth, setResourceContentWidth] = useState(0);
  ...
  const resourcePanes =
    independentResourceScroll && isWorkspace && resourceContentWidth >= 960;
  useEffect(() => {
    const measure = () => {
      setAvailable(
      ...
      const frame = root.current?.querySelector<HTMLElement>(
        ".agent-workspace-frame",
      );
      if (frame) {
        const style = window.getComputedStyle(frame);
        setResourceContentWidth(
          frame.clientWidth -
  ...
  return (
    <section
      ref={root}
      className={`agent-workbench-shell layout-${layout} module-${moduleId} ${narrow ? "is-stacked" : ""} ${resourcePanes ? "has-resource-panes" : ""}`}
      aria-label={`${title}工作区`}
      data-layout={layout}
      data-resource-scroll={
        independentResourceScroll
          ? resourcePanes
            ? "split"
            : "single"
          : undefined
    ...
    >
      ...
      >
        <section className="agent-workbench-shell__main" aria-label="主工作区">
          ...
          {showLatestControl && ownsScroll && !isWorkspace && showLatest && (
            <button
              type="button"
