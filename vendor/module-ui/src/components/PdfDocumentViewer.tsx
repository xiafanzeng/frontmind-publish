...
import { deliveryProjectHeaders, sanitizeBrandText } from "./file-preview-runtime";
import type { FilePreviewSource } from "../lib/file-preview-source";
import { usesModuleFileTransport } from "./file-preview-runtime";
import { loadModulePdfSource } from "../lib/module-file-source";
...
function OriginalPdfDocumentViewer({
  fileName,
  source,
  onClose,
  onExpired,
...
/** The original reader is shared intact. A standalone host supplies authorized
 * bytes from its V2 file boundary rather than mounting the full Dashboard proxy. */
export default function PdfDocumentViewer(props: PdfDocumentViewerProps) {
  if (!usesModuleFileTransport() || props.source.kind === "local" ||
      (props.source.kind === "external" && /^(blob:|data:)/.test(props.source.url))) {
    return <OriginalPdfDocumentViewer {...props} />;
  }
  return <ModulePdfDocumentViewer {...props} />;
}
function ModulePdfDocumentViewer(props: PdfDocumentViewerProps) {
  const sourceKey = props.source.kind === "owned_file" ? props.source.fileId : props.source.kind === "external" ? props.source.url : "local";
  const expiresAt = props.source.kind === "external" ? undefined : props.source.expiresAt;
  const [resolved, setResolved] = useState<{key: string; source: FilePreviewSource} | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [attempt, setAttempt] = useState(0);
  useEffect(() => {
    const controller = new AbortController();setError(null);setResolved(null);
    void loadModulePdfSource(props.source, props.fileName, controller.signal).then(source => {
      if (!controller.signal.aborted) setResolved({key:sourceKey,source});
    }).catch(cause => {if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : String(cause));});
    return () => controller.abort();
  }, [sourceKey, expiresAt, props.fileName, attempt]);
  if (resolved?.key === sourceKey) return <OriginalPdfDocumentViewer {...props} source={resolved.source} />;
  return <div className="flex h-full flex-col items-center justify-center gap-3 p-6" role={error ? "alert" : "status"}>
    {error ? <><p>{error}</p><Button onClick={() => setAttempt(value => value + 1)}>重试</Button></> : <><Loader2 className="h-5 w-5 animate-spin"/><p>正在读取 PDF…</p></>}
  </div>;
}
