...
import {
  ...
  answerShareUrlSchema,
  keywordEvaluationSchema,
} from "./results.js";
import { monitoringQuestionCategorySchema } from "./question-categories.js";
...
export const monitoringScopeSchema = z
  .object({
    ...
    questionCategory: monitoringQuestionCategorySchema.optional(),
...
/** Independent export selection; normal answer pagination remains single-dimension. */
export const monitoringReportScopeSchema = monitoringScopeSchema.safeExtend({
  questionIds: z.array(idSchema).min(1).max(500).transform((ids) => [...new Set(ids)]).optional(),
  platformIds: z.array(idSchema).min(1).max(100).transform((ids) => [...new Set(ids)]).optional(),
}).superRefine((value, ctx) => {
  if (value.questionId || value.platformId) ctx.addIssue({ code: "custom", message: "Use report questionIds and platformIds selections" });
});
export type MonitoringReportScope = z.infer<typeof monitoringReportScopeSchema>;
  ...
  filters: z.object({
    questions: z.array(
      z.object({
        ...
        label: z.string(),
        category: monitoringQuestionCategorySchema.nullable().default(null),
      }),
    ),
    platforms: z.array(
      z.object({
...
export const monitoringAnswersListOutputSchema = z.object({
  items: z.array(
    z.object({
      ...
      questionId: idSchema,
      question: z.string(),
      questionCategory: monitoringQuestionCategorySchema
        .nullable()
        .default(null),
...
export const monitoringAnswerDetailOutputSchema = z.object({
  ...
  questionId: idSchema,
  question: z.string(),
  questionCategory: monitoringQuestionCategorySchema.nullable().default(null),
  platform: monitoringPlatformViewOutputSchema,
  repetition: z.number().int().positive(),
  status: attemptStatusSchema,
  answerMarkdown: z.string(),
  shareUrl: answerShareUrlSchema.nullable().default(null),
  /** Immutable policy on the actual attempt, never inferred from returned media. */
  screenshotPolicy: z.union([z.literal(0), z.literal(1), z.literal(2)]).nullable().optional(),
  /** The monitored brand controls mention-only screenshots, even in competitor views. */
  screenshotBrandMentioned: z.boolean().nullable().optional(),
  keywordEvaluations: z.array(keywordEvaluationSchema).default([]),
  ...
  archivedScreenshots: z.array(
    z.object({
      ...
      thumbnailAccessPath: z
        .string()
        .startsWith("/api/monitoring/media/")
        .nullable(),
